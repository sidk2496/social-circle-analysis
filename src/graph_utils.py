...
class Graph:
    ...
    def label_propagation(self, alpha):
        ...
        for node_id, node in enumerate(self.nodes):
            min_avg_sim = 1.1
            nearest_circle_members = []
            for circle_id in node.membership:
                neighbor_indices = list(self.circles[circle_id].members)
                avg_sim = np.average(self.sim_matrix[node_id, neighbor_indices])
                if min_avg_sim > avg_sim:
                    nearest_circle_members = neighbor_indices

            weights = self.sim_matrix[node_id, list(nearest_circle_members)]
            weights /= weights.sum()
            one_indices = node.attributes == 1
            neighbor_attributes = np.vstack([self.nodes[nei_id].attributes for nei_id in nearest_circle_members])
